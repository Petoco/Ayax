    ...
    case "JOIN":
      if (stateChange.members.size === 1 && player.paused) {
        let emb = new MessageEmbed()
          .setAuthor(`Resuming paused song`, client.botconfig.IconURL)
          .setColor(client.botconfig.EmbedColor)
          .setDescription(
            `Resuming the paused song because someone have joined the voice channel`
          );
    ...
    case "LEAVE":
      if (stateChange.members.size === 0 && !player.paused && player.playing) {
        ...
        let emb = new MessageEmbed()
          ...
          .setColor(client.botconfig.EmbedColor)
          .setDescription(`All listeners leave the music channel, I have stopped the music until someone rejoins the voice channel`);
        await client.channels.cache.get(player.textChannel).send(emb);
      }
      break;
  }
