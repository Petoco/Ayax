      ...
      if (SearchString.match(client.Lavasfy.spotifyPattern)) {
        ...
        if (Searched.loadType === "PLAYLIST_LOADED") {
          ...
          SongAddedEmbed.setDescription(
            `[${Searched.tracks[0].info.title}](${Searched.tracks[0].info.uri}) by ${Searched.tracks[0].info.author}`
          );
        ...
        if (Searched.loadType === "NO_MATCHES")
          ...
          // SongAddedEmbed.setThumbnail(Searched.tracks[0].displayThumbnail());
          SongAddedEmbed.setDescription(
            `[${Searched.tracks[0].title}](${Searched.tracks[0].uri}) by ${Searched.tracks[0].author}`
          );
          if (player.queue.totalSize > 1)
            SongAddedEmbed.addField(
              "Position in queue",
        ...
        switch (res.loadType) {
          ...
          case "SEARCH_RESULT":
            ...
            if (!player.playing && !player.paused && !player.queue.length) {
              ...
              SongAddedEmbed.setColor(client.botconfig.EmbedColor);
              SongAddedEmbed.setDescription(`[${track.title}](${track.uri}) by ${track.author}`);
              if (player.queue.totalSize > 1)
                SongAddedEmbed.addField(
                  "Position in queue",
                  `${player.queue.size - 0}`,
